...
void SemanticAnalyser::run(std::unique_ptr<ProgramAST> program) {
    program->accept(*this);
}
...
void SemanticAnalyser::visit(AssignExprAST &n) {
    ...
    // If so, there is a type mismatch where we cannot directly convert between types
    if (!(isNumeric(n.getVariable()->getType()) && isNumeric(n.getExpression()->getType()))) {
        mErrorReporter.typeError(n.getSourceLocation(), typeToString(n.getVariable()->getType()) + " like variable", typeToString(n.getExpression()->getType()));
    }

    // There is a type mismatch
    // We only allow type widening (so int to float)
    if (n.getVariable()->getType() == TYPE::INT && n.getExpression()->getType() == TYPE::FLOAT) {
        mErrorReporter.conversionError(n.getSourceLocation(), typeToString(n.getVariable()->getType()), typeToString(n.getExpression()->getType()));
    }

    // TODO: Convert the expression to a float 
}
...
void SemanticAnalyser::visit(BinaryExprAST &n) {
    ...
    if (n.getLHS()->getType() == n.getRHS()->getType()) {
        ...
        n.setType(n.getLHS()->getType());
        return;
    }

    if (!(isNumeric(n.getLHS()->getType()) && isNumeric(n.getRHS()->getType()))) {
        mErrorReporter.typeError(n.getSourceLocation(), typeToString(n.getLHS()->getType()) + " like left hand side of expression", typeToString(n.getRHS()->getType()));
    }
    ...
    if (n.getLHS()->getType() == TYPE::INT && n.getRHS()->getType() == TYPE::FLOAT) {
        // TODO: convert the LHS to a float

    } else if (n.getLHS()->getType() == TYPE::FLOAT && n.getRHS()->getType() == TYPE::INT) {
        // TODO: convert the RHS to a float

    } else {
        // Should not reach here
        throw InternalCompilerError("Unknown type mismatch in binary expression semantic analysis");
    }
}
void SemanticAnalyser::visit(UnaryExprAST &n) {
    ...
    if (n.getOperator() == TOKEN_TYPE::MINUS) {
        ...
        if (isNumeric(exprType)) {
            ...
            mErrorReporter.typeError(n.getSourceLocation(), "numeric type", typeToString(exprType));
        }
        ...
        if (exprType == TYPE::BOOL) {
            ...
            mErrorReporter.typeError(n.getSourceLocation(), "boolean type", typeToString(exprType));
        }
    } else {
        // Unknown operator
        throw InternalCompilerError("Unknown unary operator in semantic analysis");
    }
}
...
void SemanticAnalyser::visit(CallExprAST &n) {
    ...
    if (!sym || sym->getKind() != IDENT_TYPE::FUNCTION) {
        mErrorReporter.typeError(n.getCallee()->getSourceLocation(), "identifier to be a function", "variable");
    }

    // Set the type of the call expression to be that of the function return type
    n.setType(sym->getType());

    FunctionDeclAST *funcDecl = static_cast<FunctionDeclAST*>(sym->getDeclaration());
    FunctionPrototypeAST *proto = funcDecl->getProto();

    // Check the number of arguments matches the number of parameters
    if (n.getArgs()->getArgsList().size() != static_cast<size_t>(proto->getSize())) {
        mErrorReporter.semanticError(
            n.getSourceLocation(),
            std::to_string(proto->getSize()) + " number of arguments to match number of parameters, but got " +
            std::to_string(n.getArgs()->getArgsList().size()) + " arguments.");   
    }
...
void SemanticAnalyser::visit(FunctionPrototypeAST &n) {
    ...
    Symbol symbol = Symbol(
        ...
        n.getType(),
        IDENT_TYPE::FUNCTION,
        nullptr // TODO: should we have pointers to the declarations ?
    );
...
void SemanticAnalyser::visit(IfExprAST &n) {
    ...
    if (n.getCondition()->getType() != TYPE::BOOL) {
        mErrorReporter.typeError(n.getCondition()->getSourceLocation(), "bool", typeToString(n.getCondition()->getType()));
    }
...
void SemanticAnalyser::visit(ReturnAST &n) {
    if (!mInsideFunction) {
        mErrorReporter.semanticError(n.getSourceLocation(), "Return statement not inside a function");
    }
    ...
    if (n.getExpression()) {
        ...
        if (!(isNumeric(exprType) && isNumeric(mCurrentReturnType))) {
            mErrorReporter.typeError(n.getSourceLocation(), typeToString(mCurrentReturnType), typeToString(exprType));
        }

        // Check if we have to convert
        if (mCurrentReturnType == TYPE::FLOAT && exprType == TYPE::INT) {
            // TODO: promote int to a float
        } else {
            // Cannot convert float to int
            mErrorReporter.conversionError(n.getSourceLocation(), typeToString(exprType), typeToString(mCurrentReturnType));
        }
    } else {
        // We have an empty return statement
        if (mCurrentReturnType != TYPE::VOID) {
            mErrorReporter.semanticError(n.getSourceLocation(), "returning a value but this function returns nothing.");
        }
    }
