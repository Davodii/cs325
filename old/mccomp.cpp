...
class GlobVarDeclAST : public DeclAST {
  ...
  public:
    GlobVarDeclAST(std::unique_ptr<VariableASTnode> var,
...
class ArgsAST : public ASTnode {
  ...
  public:
    ArgsAST(const std::string &Callee,
...
static std::unique_ptr<ASTnode> ParseElseStmt() {
    ...
    if (CurTok.type == ELSE) { // FIRST(else_stmt)
        ...
        return Else;
               ...
               CurTok.type == SC || CurTok.type == LBRA ||
               CurTok.type == WHILE || CurTok.type == IF ||
               CurTok.type == ELSE || CurTok.type == RETURN ||
...
static std::unique_ptr<ASTnode> ParseStmt() {
    ...
    if (CurTok.type == NOT || CurTok.type == MINUS || CurTok.type == PLUS ||
        ...
        CurTok.type == BOOL_LIT || CurTok.type == INT_LIT ||
        CurTok.type == FLOAT_LIT || CurTok.type == SC) { // FIRST(expr_stmt)
...
static std::vector<std::unique_ptr<ASTnode>> ParseStmtListPrime() {
    ...
    if (CurTok.type == NOT || CurTok.type == MINUS || CurTok.type == PLUS ||
        CurTok.type == LPAR || CurTok.type == IDENT ||
        CurTok.type == BOOL_LIT || CurTok.type == INT_LIT ||
        CurTok.type == FLOAT_LIT || CurTok.type == SC || CurTok.type == LBRA ||
        CurTok.type == WHILE || CurTok.type == IF || CurTok.type == ELSE ||
        CurTok.type == RETURN) { // FIRST(stmt)
    ...
    if (CurTok.type == INT_TOK || CurTok.type == FLOAT_TOK ||
        ...
        for (unsigned i = 0; i < local_decls_prime.size(); i++) {
            local_decls.push_back(std::move(local_decls_prime.at(i)));
               ...
               CurTok.type == COMMA || CurTok.type == LBRA ||
               CurTok.type == IF ||
    ...
    if (CurTok.type == EXTERN) {
        ...
        if (CurTok.type == VOID_TOK || CurTok.type == INT_TOK ||
            ...
            if (CurTok.type == IDENT) {
                ...
                if (CurTok.type == LPAR) { // found '(' - this is an extern
                    ...
                    if (CurTok.type != RPAR) // syntax error
                        return LogErrorP(CurTok, "expected ')' in closing "
                    ...
                    if (CurTok.type == SC) {
                        ...
                        return LogErrorP(CurTok,
                                         "expected ;' in ending extern "
                                         "function declaration statement");
                } else
                    return LogErrorP(
                        CurTok, "expected (' in extern function declaration");
            }
